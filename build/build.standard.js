...
var _solutionFileName = '../Breeze.Sharp.Standard.sln';
...
function msBuildSolution() {
  ...
  execCommands([rootCmd + 'Clean', rootCmd + 'Rebuild'], options, function() {
    copyFiles();
    packNuget(_nugetDir + "Breeze.Sharp/Default.standard.nuspec", function () {
...
function copyFiles() {
  ...
  if (!fs.existsSync(lib)) {
    fs.mkdirSync(lib);
    fs.mkdirSync(lib + "/netstandard2.0");
  }
  var src = _sourceDir + "bin/Release/netstandard2.0/";
  var dest = _nugetDir + "Breeze.Sharp/lib/netstandard2.0/";
...
function getBreezeSharpVersion() {
     var versionFile = fs.readFileSync( _sourceDir + 'Breeze.Sharp.Standard.csproj', { encoding: 'utf8'});
