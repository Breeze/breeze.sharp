...
namespace Breeze.Sharp.Tests {
  ...
  public class SaveTests {
    ...
    [TestMethod]
    public async Task SaveKeyGenerator()
    {
      var em1 = await TestFns.NewEm(_serviceName);
      var regionType = em1.MetadataStore.GetEntityType(typeof(Region));
      regionType.AutoGeneratedKeyType = AutoGeneratedKeyType.KeyGenerator;

      Region r = em1.CreateEntity<Region>();
      r.RegionDescription = "Test-1-" + DateTime.Now;
      Assert.IsTrue(r.EntityAspect.EntityType.AutoGeneratedKeyType == AutoGeneratedKeyType.KeyGenerator, "should be KeyGenerator before save");
      Assert.IsTrue(r.RegionID < 0, "should have a temporary key < 0");

      var sr = await em1.SaveChanges();
      Assert.IsTrue(sr.Entities.Count == 1, "should have saved one entity");
      var srRegion = sr.Entities[0];
      Assert.IsTrue(srRegion == r, "should be the same Region");
      Assert.IsTrue(r.EntityAspect.EntityType.AutoGeneratedKeyType == AutoGeneratedKeyType.KeyGenerator, "should be KeyGenerator after save");
      Assert.IsTrue(r.RegionID > 0, "should have a permanent key > 0");
